...
function TransportRouteWiseDCB() {
  const transportRouteWiseReport: any = useSelector(
  ...
  useEffect(() => {
    console.log(
      "transportRouteWiseReport ",
      transportRouteWiseReport.transportRouteWiseDcb
    );
  }, [transportRouteWiseReport]);
  ...
  const onGridReady = useCallback(
    (params: any) => {
      return transportRouteWiseReport.transportRouteWiseDcb;
    },
    [transportRouteWiseReport.transportRouteWiseDcb]
  );
  // const footerData = [
  //   transportRouteWiseReport.transportRouteWiseDcb?.data?.dcbfoot,
  // ]?.map((footerRow) => {
  //   return {
  //     totalLabel: "Total",
  //     ...transportRouteWiseReport.transportRouteWiseDcb?.data?.dcbhead?.reduce(
  ...
  return (
    <>
      ...
      <Row className="mt-2">
        {!transportRouteWiseReport.isLoading ? (
          <Col md={12}>
            <div className="ag-theme-alpine grid-table">
              <div className="btn-export">
                <button
                  onClick={() =>
                    onBtExport(
                      transportRouteWiseReport?.transportRouteWiseDcbExcel
                    )
                  }
                  disabled={
                    transportRouteWiseReport?.transportRouteWiseDcbExcel === ""
              ...
              <AgGridReact
                ...
                columnDefs={transportRouteWiseReport.transportRouteWiseDcb?.data?.dcbheader?.map(
                  (heading: string, index: number) => ({
                    headerName: heading,
                    field:
                      transportRouteWiseReport.transportRouteWiseDcb?.data
                        ?.dcbheads[index],
                  })
                )}
                rowData={
                 transportRouteWiseReport.transportRouteWiseDcb?.data?.dcbresult
                    ? transportRouteWiseReport.transportRouteWiseDcb?.data
                        ?.dcbresult
                    : []
                }
...
export default TransportRouteWiseDCB;
