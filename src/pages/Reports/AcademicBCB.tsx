...
function AcademicBCB() {
    const hostelDetailsReport: any = useSelector(
  ...
  const footerData = [hostelDetailsReport.hostelDetailsDcb?.data?.foot]?.map(
    (footerRow) => {
      return {
        totalLabel: "Total",
          ...
          <Col md={12}>
            <div className="ag-theme-alpine grid-table">
              <div className="btn-export">
                <button
                  onClick={() => onBtExport(hostelDetailsReport?.hostelDetailsDcbExcel)}
              ...
              <AgGridReact
                ...
                columnDefs={hostelDetailsReport.hostelDetailsDcb?.data?.dcbheader?.map(
                  (heading: string, index: number) => ({
                    headerName: heading,
                    field: hostelDetailsReport.hostelDetailsDcb?.data?.dcbheads[index],
                  })
                )}
                rowData={
